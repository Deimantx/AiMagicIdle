...
func _draw():
	...
	var local_weapon_pos = weapon_pos - global_position
	var local_armor_pos = armor_pos - global_position
	draw_line(local_weapon_pos, local_armor_pos, Color(0.7, 0.7, 0.8), 2)
	# Add more draw_line calls for other connections as needed

func _ready():
	call_deferred("update") # Redraw when ready 
